...
- **User Presence System**: Real-time user status (online/away/busy/offline) with multi-device support and socket-based broadcasting. Status changes via Socket.IO 'update-status' event (validates online/away/busy only, persists to DB, broadcasts to all rooms). Offline status ONLY set via explicit logout endpoint (/auth/logout), NOT on socket disconnect, ensuring status persistence across reconnections. REST endpoint POST /users/status available for fallback (persist-only, requires separate socket emit for broadcasts). Socket handler is single source of truth with validation, persistence, and broadcast in one atomic operation. Security: clients cannot spoof offline status. Friends list (GET /api/friends) displays real-time status with smart sorting (online → away → busy → offline) and contextual lastSeen messages ("Active now", "Away", "Busy", or time-based for offline users).
