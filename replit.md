...
- **User Identity Consistency**: All private chat endpoints consistently use `req.user.userId` field from JWT token for user identification, eliminating previous inconsistency between `req.user.id` and `req.user.userId` that caused data corruption (October 2025).
