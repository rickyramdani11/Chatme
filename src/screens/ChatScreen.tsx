...
export default function ChatScreen() {
  ...
  const handleAudioCall = async () => {
    // Get targetUser from navigation params or selected participant
    const callTargetUser = selectedParticipant; // Use selected participant
      ...
      if (socket) {
        ...
        if (isSupport) {
          ...
          socket.emit('join-support-room', {
            ...
            isAdmin: user?.role === 'admin', // Use user?.role directly
  ...
  useEffect(() => {
    const setupSocketListeners = (socketInstance) => {
      ...
      socketInstance.removeAllListeners('incoming-call');
      socketInstance.removeAllListeners('call-response-received');
      socketInstance.removeAllListeners('call-initiated');
      socketInstance.removeAllListeners('call-error');
      socketInstance.removeAllListeners('call-ended');
  ...
  // Effect to preserve message state when app returns to active state
  useEffect(() => {
    const preserveMessageState = () => {
      // Ensure all messages remain visible after app returns to active state
      if (chatTabs.length > 0 && activeTab >= 0 && chatTabs[activeTab]) {
        ...
        // Force update FlatList if there are messages
    ...
    // Run preserveMessageState when component mounts
  ...
  useEffect(() => {
    ...
    const handleUserJoined = (joinMessage: Message) => {
      const targetTab = chatTabs.current.find(tab => tab.id === joinMessage.roomId);
      const isPrivateChat = targetTab?.type === 'private';
      const isSupportChat = targetTab?.isSupport;
    ...
    const handleUserLeft = (leaveMessage: Message) => {
      const targetTab = chatTabs.current.find(tab => tab.id === leaveMessage.roomId);
      const isPrivateChat = targetTab?.type === 'private';
      const isSupportChat = targetTab?.isSupport;
