...
export default function ChatScreen() {
    ...
    const setupSocketListeners = (socketInstance) => {
      ...
      socketInstance.on('new-message', (newMessage: Message) => {
        ...
        if (newMessage.roomId !== currentRoomId && newMessage.sender !== userRef.current?.username) {
          // If it's a private chat, update private chat notifications
          const targetTab = chatTabsRef.current.find(tab => tab.id === newMessage.roomId);
          if (targetTab?.type === 'private') {
            setPrivateChatNotifications(prev => ({
              ...prev,
              [newMessage.roomId]: (prev[newMessage.roomId] || 0) + 1
            }));
          } else {
            // Update general unread counts for non-private chats
  ...
  const handleTabPress = (index: number) => {
    ...
    // Clear private chat notifications for the selected tab
    if (selectedRoomId && privateChatNotifications[selectedRoomId]) {
      setPrivateChatNotifications(prev => ({
        ...prev,
        [selectedRoomId]: 0
      }));
  ...
  const handleLeaveRoom = () => {
    ...
    if (socket && chatTabs[activeTab] && user) {
      ...
      // Clear private chat notifications for this room
      setPrivateChatNotifications(prev => {
        const newCounts = { ...prev };
        delete newCounts[currentRoomId];
        return newCounts;
  ...
  const handleEmojiSelect = (selectedEmoji: any) => {
    if (selectedEmoji.type === 'image' && selectedEmoji.url) {
      // Extract URL or path correctly
      const imageUrl = typeof selectedEmoji.url === 'string' ? selectedEmoji.url : null;
      const localImagePath = typeof selectedEmoji.url === 'number' ? selectedEmoji.name : null; // Assuming name is used for local

      if (imageUrl && imageUrl.startsWith('/')) {
        // Server image URL
        setMessage(prev => prev + `<img:${imageUrl}>`);
      } else if (localImagePath) {
        // Local image path
        setMessage(prev => prev + `<localimg:${localImagePath}>`);
      } else {
        // Fallback for other image types or if URL is not standard
        ...
        {chatTabs.map((tab, index) => (
          <TouchableOpacity
            ...
            style={[
              ...
              (unreadCounts[tab.id] > 0 || (tab.type === 'private' && privateChatNotifications[tab.id] > 0)) && index !== activeTab && styles.unreadIndicator
            ]}
            onPress={() => {
              handleTabPress(index); // Use the handleTabPress for consistent logic
            }}
          />
        ))}
      </View>
  ...
  const loadGifts = async () => {
    try {
      ...
      const localGifts = [
        ...
        {
          id: '1008',
          name: 'Doa Bintang', icon: '⭐', price: 10000000, type: 'animated', category: 'tas saya' },
      ];
        ...
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerIcons}>
            {chatTabs[activeTab]?.isSupport ? (
              ...
              // Room Chat Icons
              <>
                {/* Private Chat Notifications Icon */}
                <TouchableOpacity
                  style={styles.headerIcon}
                  onPress={() => setShowPrivateChatMenu(true)}
                >
                  <Ionicons name="chatbubble-ellipses-outline" size={24} color="#fff" />
                  {getTotalPrivateChatNotifications() > 0 && (
                    <View style={styles.notificationBadge}>
                      <Text style={styles.notificationBadgeText}>
                        {getTotalPrivateChatNotifications() > 99 ? '99+' : getTotalPrivateChatNotifications()}
                      </Text>
                    </View>
                  )}
                </TouchableOpacity>
          ...
          <ScrollView
            ...
            onMomentumScrollEnd={(event) => {
              ...
              if (newIndex !== activeTab && newIndex >= 0 && newIndex < chatTabs.length) {
                ...
                // Clear private chat notifications for the new active tab
                if (selectedRoomId && privateChatNotifications[selectedRoomId]) {
                  setPrivateChatNotifications(prev => ({
                    ...prev,
                    [selectedRoomId]: 0
            ...
            <View style={styles.sendToAllContainer}>
              ...
              >
                <Ionicons
                  ...
                  size={20}
                  color={sendToAllUsers ? "#4ADE80" : "#333"}
                />
                ...
                <View style={styles.newGiftItemContainer}>
                  ...
                  >
                    <View style={styles.newGiftIconContainer}>
                      {gift.image ? (
                        <Image
                          source={{ uri: `${API_BASE_URL}${gift.image}` }}
                        ...
                        (gift.name && (gift.name.toLowerCase().includes('love') || gift.name.toLowerCase().includes('ufo'))) ? (
                          <Video
                            ref={giftVideoRef}
                            source={typeof gift.animation === 'string' ? { uri: `${API_BASE_URL}${gift.animation}` } : gift.animation}
                            style={styles.giftImage}
                            resizeMode="contain"
                            shouldPlay={false}
                            isLooping={false}
                          ...
                          // For GIF animations
                          <Image
                            source={{ uri: `${API_BASE_URL}${gift.animation}` }}
                            style={styles.giftImage}
                            resizeMode="contain"
                          />
                        )
        ...
        <View style={styles.giftAnimationOverlay} pointerEvents="box-none">
          ...
          >
            ...
            {activeGiftAnimation.animation &&
             ...
             !(activeGiftAnimation.name && (activeGiftAnimation.name.toLowerCase().includes('love') || activeGiftAnimation.name.toLowerCase().includes('ufo'))) && (
              <Image
                source={{ uri: `${API_BASE_URL}${activeGiftAnimation.animation}` }}
...
const styles = StyleSheet.create({
  ...
  newGiftItem: {
    backgroundColor: '#FFFFFF', // White background for gift items
    borderRadius: 16,
    padding: 16,
    alignItems: 'center',
    borderWidth: 1,
  ...
  selectedGiftItem: {
    borderColor: '#FF8C00',
    backgroundColor: '#FFF8E1', // Light yellow background for selected
  },
  giftCoinIndicators: {
    position: 'absolute',
    top: 0,
  ...
  newGiftIconContainer: {
    ...
    marginBottom: 12,
    zIndex: 2,
    width: 60, // Consistent sizing
    height: 60, // Consistent sizing
  },
  giftImage: {
    width: 60, // Consistent sizing
    height: 60, // Consistent sizing
